...
This module bridges two previously separate workflows:
  ...
  2. A LigParGen-based small-molecule parametrization workflow that lives on
     the mu3c cluster (`/home0/layfield/Claude/ligpargen/`), which turns a
     SMILES string into an OPLS-AA (CM1A/CM1A-LBCC) `.itp`/`.gro` pair via
     `ligpargen`, and its `build_gromacs_system.py` helper, which merges
     several such ligands into one atom-type-collision-free topology
     (LigParGen restarts atom-type numbering at `opls_800` and calls every
     moleculetype `MOL` on every run, so naively #include-ing two raw
...
`build_pfas_ligands.sh` (on mu3c, in the `py37` conda env, with `ligpargen`
and `$BOSSdir` set — see LIGPARGEN_CLUSTER_GUIDE.md) is a separate,
user-initiated step.

Species
-------
The three PFAS named in the LCCMR proposal (PFOA, PFOS, PFBS) are
parametrized here as their deprotonated anions (net charge -1), matching
their dominant environmental/physiological ionization state at
near-neutral pH (their conjugate-acid pKa values are all well below 7).
LigParGen's CM1A-LBCC charge model only supports NEUTRAL molecules, so
charged species must use plain CM1A (see `charge_model` below and the
cluster guide's "CM1A vs CM1A-LBCC" note).
"""
...
# --------------------------------------------------------------------------- #
# Step 1 (remote, mu3c, NOT executed here): LigParGen molecule-list + build
...
def render_ligpargen_build_script(
    ...
    remote_ligpargen_dir: str = "/home0/layfield/Claude/ligpargen",
    remote_boss_dir: str = "/home0/layfield/boss",
    conda_env: str = "py37",
    remote_outdir: str = "pfas_ligands_gmx",
) -> str:
    """Bash script that builds all PFAS ligands into one internally-consistent
    GROMACS topology via `build_gromacs_system.py`, following
    LIGPARGEN_CLUSTER_GUIDE.md section 3 ("Multiple molecules for one GROMACS
    simulation"). Meant to run on mu3c; NOT executed by this module -- copy it
    (with the accompanying `molecules.txt`) to the cluster and run it there
...
# Generated by biochar.pfas_ligands -- run this ON mu3c, in the py37 conda
# env where `ligpargen` and BOSS are installed (LIGPARGEN_CLUSTER_GUIDE.md).
# Produces one internally-consistent, collision-free topology via
# build_gromacs_system.py (LigParGen alone restarts atom-type numbering at
# opls_800 and names every moleculetype 'MOL' on every run -- naively
# #include-ing multiple raw .gmx.itp files corrupts the system).
#
# NOT executed automatically -- run by hand once you're ready to build the
# ligands:
#   scp molecules.txt build_pfas_ligands.sh mu3c:~/some/workdir/
#   ssh mu3c
...
echo "  scp -r mu3c:~/some/workdir/{remote_outdir} ./"
"""
